...
class OfflineQueueService {
  ...
  async addAction(
    ...
    console.log(`Action queued: ${actionType}`, payload);
  ...
  async sync(): Promise<SyncResult | null> {
    ...
    if (validActions.length === 0) {
      console.log('All queued actions have exceeded max retries');
      return null;
    }
    ...
    if (!netInfo.isConnected) {
      console.log('No network connection, sync delayed');
      return null;
    }
    ...
    try {
      console.log(`Syncing ${validActions.length} offline actions...`);
  ...
  async cleanupFailedActions(): Promise<number> {
    ...
    if (removedCount > 0) {
      await this.saveQueue();
      console.log(`Cleaned up ${removedCount} failed actions from queue`);
    }
