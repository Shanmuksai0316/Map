...
export const LaundryRequestScreen = ({ navigation }: any) => {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const response = await apiService.get<{ data: LaundryRequest[] }>(
        `${APP_CONFIG.ENDPOINTS.LAUNDRY}/requests`
      );
  ...
  const handleCreateRequest = async () => {
    ...
    try {
      await apiService.post(`${APP_CONFIG.ENDPOINTS.LAUNDRY}/requests`, {
        item_counts: {
      ...
      <ScrollView
        ...
        {loading ? (
          ...
          <View style={styles.emptyState}>
            ...
            />
            <Text style={styles.emptyTitle}>No Laundry Requests</Text>
            ...
            <TouchableOpacity
              ...
              onPress={() => setShowCreateModal(true)}>
              <Text style={styles.emptyButtonText}>Request Pickup</Text>
            </TouchableOpacity>
          </View>
        ) : (
          <>
          ...
          <View style={styles.infoContent}>
            <Text style={styles.infoText}>
              • Submit laundry with item counts and weight
            </Text>
            <Text style={styles.infoText}>
              • Track status: Requested → Processing → Ready
            </Text>
            <Text style={styles.infoText}>
              • Manual verification required on handover
            </Text>
            <Text style={styles.infoText}>
                  ...
                  <View style={styles.itemsGrid}>
                    {request.item_counts.shirts > 0 && (
                      <Text style={styles.itemText}>
                        ...
                        />
                        {request.item_counts.shirts} Shirts
                      </Text>
                    )}
                    {request.item_counts.pants > 0 && (
                      <Text style={styles.itemText}>
                        <Ionicons
                          ...
                          style={styles.itemIcon}
                        />
                        {request.item_counts.pants} Pants
                      </Text>
                    )}
                    {request.item_counts.towels > 0 && (
                      <Text style={styles.itemText}>
                        <Ionicons
                          name="shirt-outline"
                          size={16}
                          ...
                          style={styles.itemIcon}
                        />
                        {request.item_counts.towels} Towels
                      </Text>
                    )}
                    {request.item_counts.bedsheets > 0 && (
                      <Text style={styles.itemText}>
                        <Ionicons
                          name="bed-outline"
                          size={16}
                          ...
                          style={styles.itemIcon}
                        />
                        {request.item_counts.bedsheets} Bedsheets
                      </Text>
                    )}
                    {request.item_counts.others > 0 && (
                      <Text style={styles.itemText}>
                        <Ionicons
                          name="cube-outline"
                          size={16}
                          ...
                          style={styles.itemIcon}
                        />
                        {request.item_counts.others} Others
                      </Text>
                    )}
                  </View>
        ...
        {completedRequests.length > 0 && (
          <View style={styles.section}>
            ...
            {completedRequests.map(request => (
              <View key={request.id} style={[styles.requestCard, styles.completedCard]}>
                <View style={styles.requestHeader}>
                  <View>
                    <Text style={styles.requestNumber}>{request.request_number}</Text>
                    <Text style={styles.requestDate}>
