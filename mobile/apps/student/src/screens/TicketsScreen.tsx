...
import { ticketSchema, type TicketFormData } from '../../validation/schemas/ticket.schema';
  ...
  const submitTicket = async (data: TicketFormData, hostelId: number) => {
    ...
    fetch('http://127.0.0.1:7242/ingest/f2a649ac-444d-418b-bdf8-72f54480be07', {
      ...
      body: JSON.stringify({
        ...
        location: 'apps/student/TicketsScreen.tsx:submitTicket',
        message: 'Submitting ticket payload',
        data: {
          apiCategory,
          title,
          hostelId,
  ...
  const onSubmit = async (data: TicketFormData) => {
    ...
    fetch('http://127.0.0.1:7242/ingest/f2a649ac-444d-418b-bdf8-72f54480be07', {
      ...
      body: JSON.stringify({
        ...
        runId: 'tickets-run-1',
        hypothesisId: 'H1_H2',
        location: 'apps/student/TicketsScreen.tsx:onSubmit',
        message: 'onSubmit called',
        data: {
          categoryFilter,
          hasDescription: !!data.description?.trim(),
          selectedHKOption,
          selectedRepairSubcategory,
          hasOthersText: !!othersText.trim(),
          hasRepairOtherText: !!repairOtherText.trim(),
          userHostelId: user?.hostel_id ?? null,
        },
        timestamp: Date.now(),
      }),
    }).catch(() => {});
      ...
      try {
        ...
        fetch('http://127.0.0.1:7242/ingest/f2a649ac-444d-418b-bdf8-72f54480be07', {
          ...
          body: JSON.stringify({
            ...
            hypothesisId: 'H2',
            location: 'apps/student/TicketsScreen.tsx:onSubmit',
            message: 'Resolved hostelId from profile',
            data: {
              resolvedHostelId,
            },
            timestamp: Date.now(),
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.header, { paddingTop: HEADER_PADDING_TOP, paddingBottom: HEADER_PADDING_BOTTOM, minHeight: HEADER_PADDING_TOP + HEADER_ROW_HEIGHT + HEADER_PADDING_BOTTOM }]}>
        <View style={[styles.headerRow, { height: HEADER_ROW_HEIGHT }]}>
          ...
          <Text style={styles.headerTitle}>
            {categoryFilter === 'housekeeping' ? 'House Keeping' : 
             categoryFilter === 'repair_maintenance' ? 'Repair & Maintenance' : 
             'Tickets'}
          </Text>
          <TouchableOpacity
      ...
      <ScrollView
        ...
        {loading ? (
          ...
          tickets
            .filter((ticket) => !categoryFilter || ticket.request_type === categoryFilter)
            .map((ticket) => (
            <TouchableOpacity
              key={ticket.id}
      ...
      <Modal
        ...
        <View style={styles.modalContainer}>
          ...
          <ScrollView style={styles.modalContent}>
            {categoryFilter && (
              <View style={[styles.inputGroup, styles.requestTypeInfo]}>
                <Ionicons
              ...
              <View style={styles.inputGroup}>
                ...
                <View style={styles.radioGroup}>
                  {HOUSEKEEPING_OPTIONS.map((option) => (
                    <TouchableOpacity
                      key={option}
                      style={[styles.radioOption, selectedHKOption === option && styles.radioOptionSelected]}
                      onPress={() => {
                        hapticService.onButtonPress();
                        setSelectedHKOption(option);
                        setValue('issue', option === 'Others' ? othersText || option : option);
                        if (option === 'Others') setShowOthersModal(true);
                      }}>
                      <View style={styles.radioCircle}>
                        {selectedHKOption === option && <View style={styles.radioCircleInner} />}
                      </View>
                      <Text style={[styles.radioText, selectedHKOption === option && styles.radioTextSelected]}>{option}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
                {selectedHKOption === 'Others' && (
                  <View style={[styles.inputGroup, { marginTop: theme.spacing.sm }]}>
                    <Text style={styles.inputLabel}>Describe the service needed *</Text>
                    <TextInput
              ...
              <View style={styles.inputGroup}>
                ...
                <View style={styles.radioGroup}>
                  {REPAIR_SUBCATEGORIES.map((option) => (
                    <TouchableOpacity
                      key={option}
                      style={[styles.radioOption, selectedRepairSubcategory === option && styles.radioOptionSelected]}
                      onPress={() => {
                        hapticService.onButtonPress();
                        setSelectedRepairSubcategory(option);
                        setValue('issue', option === 'Other' ? repairOtherText || option : option);
                        if (option === 'Other') setRepairOtherText('');
                      }}>
                      <View style={styles.radioCircle}>
                        {selectedRepairSubcategory === option && <View style={styles.radioCircleInner} />}
                      </View>
                      <Text style={[styles.radioText, selectedRepairSubcategory === option && styles.radioTextSelected]}>{option}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
                {selectedRepairSubcategory === 'Other' ? (
                  <View style={[styles.inputGroup, { marginTop: theme.spacing.sm }]}>
                    ...
                    <TextInput
                      ...
                      onChangeText={(t) => {
                        setRepairOtherText(t);
                        setValue('issue', t || 'Other');
                      }}
                      multiline
                      numberOfLines={2}
                      textAlignVertical="top"
