...
export const GuardGatePassScreen = ({ navigation }: any) => {
  ...
  const fetchData = async () => {
    try {
      const [outpassData, leaveData, guestData] = await Promise.all([
        apiService.get<{ data: Outpass[] }>(APP_CONFIG.ENDPOINTS.GUARD_OUTPASSES_ACTIVE).catch(() => ({ data: [] })),
        apiService.get<{ data: Leave[] }>(APP_CONFIG.ENDPOINTS.GUARD_LEAVES_ACTIVE).catch(() => ({ data: [] })),
        apiService.get<{ data: GuestEntry[] }>(APP_CONFIG.ENDPOINTS.GUARD_GUEST_ENTRIES_ACTIVE).catch(() => ({ data: [] })),
      ]);
