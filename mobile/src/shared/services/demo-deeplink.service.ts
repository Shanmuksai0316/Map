...
export class DemoDeepLinkService {
  ...
  async handleUrl(url: string): Promise<boolean> {
    try {
      ...
      if (urlObj.host === 'auth' && urlObj.pathname === '/demo') {
        ...
        await StorageService.set(APP_CONFIG.STORAGE_KEYS.AUTH_TOKEN, token);
        await StorageService.setObject(APP_CONFIG.STORAGE_KEYS.USER_DATA, user);
        await StorageService.set(APP_CONFIG.STORAGE_KEYS.USER_ROLE, role);
        if (userPhone) {
          await StorageService.set(APP_CONFIG.STORAGE_KEYS.PHONE_NUMBER, userPhone);
        }
  ...
  /**
   * Handle initial URL explicitly (used when initialization needs an immediate check)
   */
