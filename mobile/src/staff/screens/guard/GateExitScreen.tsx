 ...
 * Gate Exit Screen
 * 
 * Allows Guards to record student exit (OUT) at the gate.
...
export const GateExitScreen = ({ navigation }: any) => {
  ...
  const handleGateExit = async () => {
    ...
    if (now > validUntil) {
      Alert.alert('Error', 'Gate pass has expired');
      return;
    }
    ...
    try {
      ...
      if (isOnline) {
        // Direct API call
        await apiService.post('/gate/entries', exitData);
        ...
        await addAction('gate_out', exitData);
        Alert.alert('Offline', 'Exit queued for sync when online');
        setSelectedStudent(null);
        setGatePass(null);
        setSearchQuery('');
      }
      ...
      Alert.alert('Error', 'Failed to record exit. Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <ScrollView
      ...
      <StaffScreenHeader onBack={() => navigation.goBack()} showBell={false}  title="Gate Exit" />
            ...
            <View style={styles.gatePassInfo}>
              ...
              {gatePass.overnight && (
                <View style={styles.overnightBadge}>
                  ...
                  <Text style={styles.overnightText}>Overnight Pass</Text>
                </View>
              )}
            </View>
          ) : (
      ...
      {selectedStudent && gatePass && (
        <View style={styles.actionSection}>
          <GradientButton
            style={[styles.exitButton, submitting && styles.exitButtonDisabled]}
            onPress={handleGateExit}
            disabled={submitting}>
            {submitting ? (
