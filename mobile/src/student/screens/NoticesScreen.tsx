...
export const NoticesScreen = ({ navigation }: any) => {
  ...
  const [filter, setFilter] = useState<'all' | 'general' | 'event'>('all');
  ...
  const handleImagePress = (notice: Notice, imageIndex: number) => {
    if (notice.images && notice.images.length > 0) {
      setLightboxImages(notice.images);
        ...
        <View style={styles.imagesContainer}>
          {notice.images.length === 1 ? (
            ...
            <View style={styles.twoImagesRow}>
              {notice.images.slice(0, 2).map((img, idx) => (
                <Image
                  key={idx}
                  source={{ uri: img }}
                  style={styles.twoImagesItem}
                  resizeMode="cover"
                />
              ))}
            </View>
            ...
            <View style={styles.multipleImagesGrid}>
              {notice.images.slice(0, 4).map((img, idx) => (
                <Image
                  key={idx}
                  source={{ uri: img }}
