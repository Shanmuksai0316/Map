...
@php
    ...
    $formatInt = function ($value): string {
        ...
        // Avoid \Illuminate\Support\Number::format() because it requires ext-intl.
...
>
        ...
        <span class="fi-pagination-overview text-sm font-medium text-gray-700 dark:text-gray-200">
            {{
                trans_choice(
                    ...
                    $paginator->total(),
                    [
                        'first' => $formatInt($paginator->firstItem() ?? 0),
                        'last' => $formatInt($paginator->lastItem() ?? 0),
                        'total' => $formatInt($paginator->total()),
                    ],
                )
            }}
        </span>
    ...
    @if ((! $isSimple) && $paginator->hasPages())
        <ol class="fi-pagination-items justify-self-end rounded-lg bg-white shadow-sm ring-1 ring-gray-950/10 dark:bg-white/5 dark:ring-white/20">
            @if (! $paginator->onFirstPage())
                @if ($extremeLinks)
