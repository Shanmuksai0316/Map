...
- Release tools: Fastlane for App Store and Play Store releases
- E2E tools: Playwright for web testing, Maestro for mobile flow testing
...
- Web E2E tests pass: `npx playwright test`
- Mobile E2E flows pass: `maestro test <flow-file-or-folder>`
...
4. Build and sign mobile releases using Fastlane lanes.
5. Submit app builds to Play Store and App Store via Fastlane.
6. Deploy backend/web via script/automation.
7. Run migrations safely.
8. Run post-deploy health checks.
9. Validate critical user journeys (login, role dashboard, primary transaction).
10. Validate integrations (SMS, push, email, storage).
11. Publish release notes and UAT evidence.
...
- Use Fastlane as the default release pipeline for Android/iOS stores.
...
fastlane android beta   # or release lane
fastlane ios beta       # or release lane
```
